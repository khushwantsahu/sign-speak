...
# Complete database of ASL alphabet (A-Z) and basic vocabulary using placeholders
SIGN_DATABASE = {
    "a": {
...
async def get_gesture_description(term: str) -> str:
    ...
    if clean_term in SIGN_DATABASE:
        ...
        video_str = f"\n\n📺 **Video Tutorial:** {data['video_link']}" if "video_link" in data else ""
        return (
        ...
        if len(clean_term) == 1 and clean_term.isalpha():
            # Dynamically return a placeholder for the missing alphabet letter
            return (
                f"🤟 **Sign Description for '{term.upper()}'** (alphabet):\n"
                f"To sign the letter '{term.upper()}', please check the visual guide.\n\n"
                f"**Visual Reference:**\n[VISUAL: {clean_term}]\n\n"
                f"📺 **Video Tutorial:** [VIDEO: alphabet]"
            )
        return (
